...
import { 
  ...
  ListItemText, BottomNavigation, BottomNavigationAction, Paper,
  Modal,Box,TextField,Button,IconButton,Stack,Snackbar,Alert,
  Slider,Grid,Divider
} from "@mui/material";
import { 
...
import 'react-image-crop/dist/ReactCrop.css';
// Main App component that contains both Sidebar and PostCreation
const App = () => {
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      <Sidebar onCreateClick={handleCreateClick} selectedTab={selectedTab} setSelectedTab={setSelectedTab} />
      
      <Box component="main" sx={{ flexGrow: 1, p: 3, ml: { sm: '250px' } }}>    
        <PostCreationModal
...
// Sidebar component
const Sidebar = ({ onCreateClick, selectedTab, setSelectedTab }) => {
  const isMobile = useMediaQuery("(max-width:800px)");

  const menuItems = [
    { label: "Home", icon: <Home /> },
    { label: "Search", icon: <Search /> },
    { label: "Create", icon: <AddBox />, action: onCreateClick }, // Ensure onCreateClick is properly passed
    { label: "Notifications", icon: <FavoriteBorder /> },
    { label: "Profile", icon: <Person /> },
  ];

  const handleMenuClick = (index) => {
    console.log(index);
    setSelectedTab(index); // Update selected tab state
    if (menuItems[index].action) {
      menuItems[index].action(); // Call the action if it exists (e.g., onCreateClick)
    }
  };
  ...
  return (
    <>
      {isMobile ? (
        <Paper sx={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 1000, borderTop: "1px solid #ddd" }} elevation={3}>
          <BottomNavigation
            value={selectedTab}
            onChange={(event, newValue) => handleMenuClick(newValue)} // Fix: Ensure tab selection updates state
          ...
          >
            {menuItems.map((item, index) => (
              <BottomNavigationAction
                ...
                label={item.label}
                icon={item.icon}
                sx={{ color: selectedTab === index ? "black" : "gray" }}
              />
            ))}
          </BottomNavigation>
        </Paper>
        ...
        <Drawer
          variant="permanent"
          anchor="left"
          sx={{ width: 250, flexShrink: 0, "& .MuiDrawer-paper": { width: 250, boxSizing: "border-box", padding: 2 } }}
        >
          <Typography variant="h5" sx={{ fontStyle: "italic", textAlign: "center", marginBottom: 2 }}>
            PocketWriter
          </Typography>
          <List>
            {menuItems.map((item, index) => (
              <ListItem
                ...
                sx={{ "&.Mui-selected": { backgroundColor: "#f0f0f0", borderRadius: "10px" } }}
              >
